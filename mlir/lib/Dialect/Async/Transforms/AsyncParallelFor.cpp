    ...
    return [&, loopIdx](OpBuilder &nestedBuilder, Location loc, Value iv,
      ...
      if (loopIdx < op.getNumLoops() - 1) {
        ...
        if (loopIdx + 1 >= op.getNumLoops() - numBlockAlignedInnerLoops) {
          ...
          loopResFwd = nb.create<scf::ForOp>(c0, tripCounts[loopIdx + 1], c1,
          ...
          loopResFwd = nb.create<scf::ForOp>(lb, ub, c1, args,
                                             workLoopBuilder(loopIdx + 1))
                           .getResults();
        }

        nb.create<scf::YieldOp>(loc, loopResFwd);
        return;
      }
...
AsyncParallelForRewrite::matchAndRewrite(scf::ParallelOp op,
  ...
  auto dispatch = [&](OpBuilder &nestedBuilder, Location loc) {
    ...
    // less than the `maxIterations` value.
    ...
    // is reduced because the problem typically becomes memory bound. For
    // small number of threads it helps with stragglers.
